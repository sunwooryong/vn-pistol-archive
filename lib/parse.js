...
function parseEvent(rawEvent) {
  ...
  // --- 혼성/성별 ---
  //   "nam nữ"(남녀 통합 릴레이): 남·여가 한 종목에 함께 사격 → 선수 개인 성별로 분리해야 함.
  //   (주의: "nữ" 검사가 "nam nữ"에도 먼저 걸리므로 통합 여부를 우선 판별한다)
  const hasNam = /\bnam\b/.test(e), hasNu = /\bnu\b/.test(e);
  if (/hon hop/.test(e)) result.gender = 'X';
  else if (hasNam && hasNu) { result.gender = null; result.bothGenders = true; }  // 선수별로 setGender 처리
...
function buildEventCode(result) {
  ...
  const base = result.distance === '10m' ? 'AP10M'
