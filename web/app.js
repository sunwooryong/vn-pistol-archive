...
async function renderMyAthletesComps(box, year) {
  ...
  const eb = $('#team-eval-btn'); if (eb) eb.onclick = () => openReport(cards.map(c => c.a), 'all');
...
// 선수 평가 보고서 (연말 총결산) — 연도 선택 · 단일/다중 선수 · 인쇄
async function openEvalReport(athletes, year) {
  athletes = (athletes || []).filter(Boolean);
  if (!athletes.length) return;
  let view = document.getElementById('report-view'); if (view) view.remove();
  view = el('div', 'report-view'); view.id = 'report-view';
  view.innerHTML = `<div class="rv-bar no-print">
      <label class="rv-yr">${t('연도')} <select id="ev-year"></select></label>
      <div class="rv-athsel no-print" id="ev-athsel"></div>
      <button class="rv-print" id="rv-print">🖨️ ${t('인쇄')}</button>
      <button class="rv-close" id="rv-close">${t('닫기')}</button></div>
    <div class="rv-doc" id="rv-doc"><div class="muted" style="padding:40px;text-align:center">${t('불러오는 중…')}</div></div>`;
  document.body.appendChild(view);
  ...
  const careers = new Map(); const yearsSet = new Set();
  for (const a of athletes) { const rows = await DB.athleteCareer(a.id); careers.set(a.id, rows); rows.forEach(r => yearsSet.add(r.competition.year)); }
  const years = [...yearsSet].sort((x, y) => y - x);
  let curYear = years.includes(year) ? year : (years[0] || year);
  const ysel = view.querySelector('#ev-year');
  ysel.innerHTML = years.map(y => `<option ${y === curYear ? 'selected' : ''}>${y}</option>`).join('');
  ysel.onchange = () => { curYear = +ysel.value; render(); };

  const selIds = new Set(athletes.map(a => a.id));
  if (athletes.length > 1) {
    const box = view.querySelector('#ev-athsel');
    box.innerHTML = `<span class="rv-glab">${t('선수')}:</span>` + athletes.map(a => `<label class="ev-chk"><input type="checkbox" checked data-id="${a.id}"> ${esc(a.full_name)}</label>`).join('');
    box.querySelectorAll('input').forEach(c => c.onchange = () => { if (c.checked) selIds.add(+c.dataset.id); else selIds.delete(+c.dataset.id); render(); });
  }

  async function render() {
    const doc = view.querySelector('#rv-doc');
    doc.innerHTML = `<div class="muted" style="padding:30px;text-align:center">${t('계산 중…')}</div>`;
    const parts = [];
    for (const a of athletes) { if (!selIds.has(a.id)) continue; parts.push(await renderAthleteEval(a, careers.get(a.id) || [], curYear)); }
    doc.innerHTML = parts.join('') || `<div class="muted" style="padding:30px;text-align:center">${t('선수를 선택하세요.')}</div>`;
  }
  render();
}
window.openEvalReport = openEvalReport;

// 선수 종합 프로파일(도시에) — 고밀도 전문 리포트 · 인쇄 가능
async function openDossier(a, rows) {
  rows = rows || [];
  let view = document.getElementById('report-view'); if (view) view.remove();
  view = el('div', 'report-view'); view.id = 'report-view';
  view.innerHTML = `<div class="rv-bar no-print">
      <span class="rv-ttl">📊 ${t('종합 프로파일')}</span>
      <button class="rv-print" id="rv-print">🖨️ ${t('인쇄')}</button>
      <button class="rv-close" id="rv-close">${t('닫기')}</button></div>
    <div class="rv-doc dos-doc" id="rv-doc"><div class="muted" style="padding:40px;text-align:center">${t('계산 중…')}</div></div>`;
  document.body.appendChild(view);
  view.querySelector('#rv-close').onclick = () => view.remove();
  view.querySelector('#rv-print').onclick = () => window.print();
  ...
  const dstr = (() => { const d = new Date(); return `${d.getFullYear()}. ${d.getMonth() + 1}. ${d.getDate()}`; })();
  const dsort = r => (r.match_date || r.competition.date_start || '');
  const spark = (vals, w = 120, h = 26) => {
    ...
    const n = v.length, xm = (n - 1) / 2, ym = v.reduce((s, x) => s + x, 0) / n; let nu = 0, de = 0; v.forEach((y, x) => { nu += (x - xm) * (y - ym); de += (x - xm) ** 2; });
  ...
  let fin = 0, dom = 0, intl = 0; const comps = new Set(); const yset = new Set();
  rows.forEach(r => { if (r.medal) im[r.medal]++; if (r.team_medal) tm[r.team_medal]++; if (r.final_score != null) fin++; comps.add(r.competition.name + r.competition.year); yset.add(r.competition.year); if (r.competition.scope === 'international') intl++; else dom++; });
  const gamesInd = rows.filter(r => r.event.team_type === 'individual' && scored(r)).length;
  const years = [...yset].sort();
  const g = a.gender, by = a.birth_year;

  // 종목별(개인) 집계
  const byD = new Map();
  rows.forEach(r => { if (r.event.team_type !== 'individual' || !scored(r)) return; const k = r.event.discipline; (byD.get(k) || byD.set(k, []).get(k)).push(r); });
  const discRows = [...byD.entries()].map(([k, arr]) => {
    const srt = arr.slice().sort((x, y) => dsort(x).localeCompare(dsort(y)));
    const vs = srt.map(r => r.qual_total); const n = vs.length;
    const avg = vs.reduce((s, v) => s + v, 0) / n, best = Math.max(...vs), worst = Math.min(...vs);
    const sd = n > 1 ? Math.sqrt(vs.reduce((s, v) => s + (v - avg) ** 2, 0) / n) : null;
    const pbRow = srt.find(r => r.qual_total === best);
    return { k, n, avg, best, worst, sd, spark: spark(vs.slice(-16)), pbDate: pbRow ? dsort(pbRow).slice(0, 10) : '' };
  }).sort((x, y) => (CAL_ORDER.indexOf(x.k)) - (CAL_ORDER.indexOf(y.k)));

  // 비동기 데이터
  let R = null, stages = [], TR = null;
  try { R = await DB.regionalAnalysis(a.id, RANK_YEAR); } catch (e) { }
  try { stages = await DB.stageAnalysis(a.id, RANK_YEAR); } catch (e) { }
  try { TR = await DB.trainingOf(a.full_name); } catch (e) { }

  const tile = (lab, val, sub) => `<div class="dos-tile"><i>${lab}</i><b>${val}</b>${sub ? `<s>${sub}</s>` : ''}</div>`;
  const medStr = m => [m.gold && `<span class="medal gold">${MEDAL.gold}</span>${m.gold}`, m.silver && `<span class="medal silver">${MEDAL.silver}</span>${m.silver}`, m.bronze && `<span class="medal bronze">${MEDAL.bronze}</span>${m.bronze}`].filter(Boolean).join(' ') || '–';

  // 헤더
  let h = `<div class="dos-head">
    <div class="dos-rings">${window.ringsSVG || ''}</div>
    <div class="dos-htxt">
      <div class="dos-kick">${t('사격 · 베트남 사격연맹')} · ${t('종합 프로파일')}</div>
      <h1 class="dos-name">${esc(a.full_name)}${g ? ` <span class="g g-${g}">${GENDER[g]}</span>` : ''}</h1>
      <div class="dos-meta">${by ? `${by}${t('년생')} · ${t('만')} ${NOW_YEAR - by}${t('세')}` : ''}${a.unit ? ` · ${esc(a.unit)}` : ''} · ${a.is_foreign ? esc(a.nationality || '') : t('베트남')}${a.identity_key ? ` · <span class="dos-id">${esc((a.identity_key || '').split('|').slice(0, 2).join(' · '))}</span>` : ''}</div>
    </div>
    <div class="dos-asof"><i>${t('발급일')}</i><b>${dstr}</b><s>${years.length ? years[0] + '–' + years[years.length - 1] : ''}</s></div>
  </div>`;

  // KPI 타일
  h += `<div class="dos-kpis">
    ${tile(t('총 경기'), gamesInd, `${comps.size} ${t('개 대회')}`)}
    ${tile(t('개인 메달'), im.gold + im.silver + im.bronze, medStr(im))}
    ${tile(t('단체 메달'), tm.gold + tm.silver + tm.bronze, medStr(tm))}
    ${tile(t('결선 진출'), fin + t('회'), '')}
    ${tile(t('종목'), byD.size, '')}
    ${tile(t('국내') + '/' + t('국제'), dom + '/' + intl, '')}
    ${tile(t('활동'), (years.length ? (years[years.length - 1] - years[0] + 1) : 0) + t('년'), years.length + t('시즌'))}
    ${TR ? tile(t('훈련 세션'), TR.sessions.length, '') : ''}
  </div>`;

  // 종목별 성적
  h += `<div class="dos-sec">🎯 ${t('종목별 성적')} <span>${t('개인전 · 전 기간')}</span></div>
  ...
  discRows.forEach(d => {
    ...
    h += `<tr><td class="dos-nm">${esc(DISC[d.k] || d.k)}</td><td>${d.n}</td><td class="dos-b">${fmt(d.avg)}</td>
      <td class="dos-pb">${fmt(d.best)}<s>${d.pbDate ? d.pbDate.replace(/-/g, '.').slice(2) : ''}</s></td>
      <td>${fmt(d.worst)}</td><td>${d.sd != null ? '±' + d.sd.toFixed(1) : '–'}</td>
      <td class="dos-sp">${d.spark}</td>
      <td>${dz ? `<b>${dz.natRank}</b>/${dz.natN} <s>${dz.pct}%</s>` : '–'}</td></tr>`;
  });
  h += `</tbody></table>`;

  // 전국/지역 위상
  if (R && R.disciplines && R.disciplines.length) {
    const m = R.disciplines[0];
    h += `<div class="dos-sec">📍 ${RANK_YEAR} ${t('전국 위상')} <span>${esc(DISC[m.disc] || m.disc)}${R.region ? ' · ' + esc(R.region) : ''}</span></div>
      <div class="dos-kpis sm">
        ${tile(t('전국 순위'), `${m.natRank}<s2>/${m.natN}</s2>`, `${t('상위')} ${m.pct}%`)}
        ${m.regN ? tile(t('지역 순위'), `${m.regRank}<s2>/${m.regN}</s2>`, esc(m.regionTop || '')) : ''}
        ${tile(t('내 평균'), fmt(m.my), '')}
      </div>`;
    if (m.rivals && m.rivals.length) {
      h += `<table class="dos-tab compact"><thead><tr><th>${t('주요 경쟁자')}</th><th>${t('소속')}</th><th>${t('최고')}</th></tr></thead><tbody>` +
        m.rivals.slice(0, 5).map(rv => `<tr><td class="dos-nm">${esc(rv.name)}</td><td>${esc(rv.region || rv.unit || '')}</td><td class="dos-b">${fmt(rv.q != null ? rv.q : rv.best_qual)}</td></tr>`).join('') + `</tbody></table>`;
    }
  }
  ...
  if (stg.length) {
    h += `<div class="dos-sec">📊 ${t('단계 분석')} <span>${RANK_YEAR}</span></div><table class="dos-tab"><thead><tr><th>${t('종목')}</th>` +
      `<th>${t('단계')}</th><th>${t('평균')}</th><th>${t('최고')}</th><th>${t('전국')}</th></tr></thead><tbody>`;
    stg.forEach(dz => dz.stages.forEach((s, i) => {
      const weak = dz.stages.reduce((mn, x) => x.avg < mn.avg ? x : mn);
      h += `<tr><td class="dos-nm">${i === 0 ? esc(DISC[dz.disc] || dz.disc) : ''}</td>
        <td class="${s === weak ? 'dos-weak' : ''}">${esc(t(s.key))}${s === weak ? ' ▼' : ''}</td>
        <td class="dos-b">${fmt(s.avg)}</td><td>${fmt(s.best)}</td><td>${s.natN ? `${s.natRank}/${s.natN}` : '–'}</td></tr>`;
    }));
    h += `</tbody></table>`;
  }

  // 훈련 분석
  if (TR && TR.sessions && TR.sessions.length) {
    const S = TR.sessions;
    const per10 = s => s.avg != null ? s.avg : (s.series && s.series.length ? s.total / s.series.length : null);
    const byDk = new Map(); S.forEach(s => { const k = s.dk || s.disc; (byDk.get(k) || byDk.set(k, []).get(k)).push(s); });
    const compP10 = new Map(); const cAg = new Map();
    rows.forEach(r => { if (r.event.team_type !== 'individual' || !scored(r)) return; const ns = r.event.n_series || 6; const o = cAg.get(r.event.discipline) || { s: 0, n: 0 }; o.s += r.qual_total / ns; o.n++; cAg.set(r.event.discipline, o); });
    cAg.forEach((o, d) => compP10.set(d, o.s / o.n));
    const mt = S.filter(s => s.is_match).length;
    h += `<div class="dos-sec">🏋️ ${t('훈련 분석')} <span>${S.length}${t('세션')} · ${t('훈련')} ${S.length - mt}·${t('실전')} ${mt}</span></div>
      <table class="dos-tab"><thead><tr><th>${t('종목')}</th><th>${t('세션')}</th><th>${t('세션 평균')}</th><th>${t('최고')}</th><th>σ</th><th>${t('대회 평균')}</th><th>${t('대회 대비')}</th><th>${t('추이')}</th></tr></thead><tbody>`;
    [...byDk.entries()].sort((x, y) => y[1].length - x[1].length).forEach(([k, arr]) => {
      const vs = arr.map(per10).filter(v => v != null); if (!vs.length) return;
      const avg = vs.reduce((s, v) => s + v, 0) / vs.length, best = Math.max(...vs);
      const sd = vs.length > 1 ? Math.sqrt(vs.reduce((s, v) => s + (v - avg) ** 2, 0) / vs.length) : null;
      const cA = compP10.get(k), delta = cA != null ? avg - cA : null;
      const lbl = DISC[k] || (k === 'std25' ? t('25m 정밀') : k);
      h += `<tr><td class="dos-nm">${esc(lbl)}</td><td>${arr.length}</td><td class="dos-b">${fmt(avg)}</td><td>${fmt(best)}</td><td>${sd != null ? '±' + sd.toFixed(1) : '–'}</td>
        <td>${cA != null ? fmt(cA) : '–'}</td><td class="${delta >= 0 ? 'up' : 'dn'}">${delta != null ? (delta >= 0 ? '+' : '') + delta.toFixed(1) : '–'}</td>
        <td class="dos-sp">${spark(vs.slice(-16))}</td></tr>`;
    });
    h += `</tbody></table>`;
  }

  // 연도별 성적
  h += `<div class="dos-sec">📅 ${t('연도별 성적')}</div><table class="dos-tab"><thead><tr><th>${t('연도')}</th><th>${t('경기')}</th><th>${t('개인 메달')}</th><th>${t('단체 메달')}</th><th>${t('결선 진출')}</th></tr></thead><tbody>`;
  years.slice().reverse().forEach(y => {
    const yr = rows.filter(r => r.competition.year === y);
    const yi = { gold: 0, silver: 0, bronze: 0 }, yt = { gold: 0, silver: 0, bronze: 0 }; let yf = 0;
    yr.forEach(r => { if (r.medal) yi[r.medal]++; if (r.team_medal) yt[r.team_medal]++; if (r.final_score != null) yf++; });
    h += `<tr><td class="dos-b">${y}</td><td>${yr.filter(r => r.event.team_type === 'individual' && scored(r)).length}</td><td>${medStr(yi)}</td><td>${medStr(yt)}</td><td>${yf}${t('회')}</td></tr>`;
  });
  h += `</tbody></table>`;

  // 대회 기록 전체
  h += `<div class="dos-sec">🗂️ ${t('대회 기록')} <span>${rows.length}${t('건')}</span></div>
    <table class="dos-tab log"><thead><tr><th>${t('날짜')}</th><th>${t('대회')}</th><th>${t('종목')}</th><th>${t('본선')}</th><th>${t('결선')}</th><th>${t('메달')}</th><th>${t('순위')}</th></tr></thead><tbody>`;
  rows.slice().sort((x, y) => dsort(y).localeCompare(dsort(x))).forEach(r => {
    h += `<tr><td>${(dsort(r) || '').slice(2, 10).replace(/-/g, '.')}</td>
      <td class="dos-nm">${esc(r.competition.name)} <span class="scope ${r.competition.scope}">${SCOPE[r.competition.scope] || ''}</span></td>
      <td>${esc(eventLabel(r.event))}</td>
      <td class="dos-b">${r.qual_total != null ? fmt(r.qual_total) : (r.is_dnf ? 'DNF' : '–')}</td>
      <td>${r.final_score != null ? r.final_score : ''}</td>
      <td class="dos-md">${r.medal ? medalBadge(r.medal) : ''}${r.team_medal ? medalBadge(r.team_medal) + `<s>${t('단체')}</s>` : ''}</td>
      <td>${r.placement ? r.placement + t('위') : ''}</td></tr>`;
  });
  h += `</tbody></table>`;

  h += `<div class="dos-foot"><div class="dos-note">${t('본 보고서는 베트남 사격연맹 공개 기록을 기준으로 자동 작성되었습니다.')} · ${t('발급일')} ${dstr}</div><div class="rc-sign"><span class="rc-signname">RYONG</span><span class="rc-signlab">${t('작성')}</span></div></div>`;
  doc.innerHTML = h;
}
window.openDossier = openDossier;

// ── 통합 평가 리포트 (종합 프로파일 + 평가 + 시리즈/월별/연도별 심층분석) ──
const REPORT_SPARK = (vals, w = 120, h = 26) => {
  const v = vals.filter(x => x != null); if (v.length < 2) return '';
  const mn = Math.min(...v), mx = Math.max(...v), rng = mx - mn || 1;
  const pts = v.map((x, i) => `${(i / (v.length - 1) * w).toFixed(1)},${(h - (x - mn) / rng * (h - 4) - 2).toFixed(1)}`).join(' ');
  const n = v.length, xm = (n - 1) / 2, ym = v.reduce((s, x) => s + x, 0) / n; let nu = 0, de = 0; v.forEach((y, x) => { nu += (x - xm) * (y - ym); de += (x - xm) ** 2; });
...
// 큰 라인 차트 (대회 본선 추이): 점마다 점수(위)·날짜(아래) 라벨, 메달 색 점, min/mid/max 눈금
function REPORT_LINE(pts, w = 700, h = 188) {
  if (!pts || pts.length < 2) return '';
  const vals = pts.map(p => p.val), mn = Math.min(...vals), mx = Math.max(...vals), rng = mx - mn || 1;
  const L = 30, Rp = 12, T = 24, B = 40;
  const X = i => L + i / (pts.length - 1) * (w - L - Rp), Y = v => T + (1 - (v - mn) / rng) * (h - T - B);
  const mc = { gold: '#b8860b', silver: '#8a8f98', bronze: '#a9622f' };
  const dd = d => (d || '').slice(2).replace(/-/g, '.');
  const step = pts.length > 13 ? 2 : 1;
  let g = '';
  [mx, (mn + mx) / 2, mn].forEach(gv => { const y = Y(gv); g += `<line x1="${L}" y1="${y.toFixed(1)}" x2="${w - Rp}" y2="${y.toFixed(1)}" stroke="#eef1f6"/><text x="${L - 4}" y="${(y + 3).toFixed(1)}" text-anchor="end" font-size="9" fill="#c2c9d4">${gv.toFixed(0)}</text>`; });
...
async function buildReportSection(a, allRows, year) {
  ...
  const fmt = n => n == null ? '–' : (Number.isInteger(n) ? String(n) : n.toFixed(1));
  const dsort = r => (r.match_date || r.competition.date_start || '');
  const sd = vs => vs.length > 1 ? Math.sqrt(vs.reduce((s, v) => s + (v - vs.reduce((p, q) => p + q, 0) / vs.length) ** 2, 0) / vs.length) : null;
  const allYears = [...new Set(allRows.map(r => r.competition.year))].sort();
  const rows = year === 'all' ? allRows : allRows.filter(r => r.competition.year === year);
  const anYear = year === 'all' ? RANK_YEAR : year;
  const g = a.gender, by = a.birth_year;
  const dstr = (() => { const d = new Date(); return `${d.getFullYear()}. ${d.getMonth() + 1}. ${d.getDate()}`; })();
  const periodLab = year === 'all' ? `${t('전체 기간')}${allYears.length ? ` · ${allYears[0]}–${allYears[allYears.length - 1]}` : ''}` : `${year}`;
  const tile = (lab, val, sub) => `<div class="dos-tile"><i>${lab}</i><b>${val}</b>${sub ? `<s>${sub}</s>` : ''}</div>`;
  const medStr = m => [m.gold && `<span class="medal gold">${MEDAL.gold}</span>${m.gold}`, m.silver && `<span class="medal silver">${MEDAL.silver}</span>${m.silver}`, m.bronze && `<span class="medal bronze">${MEDAL.bronze}</span>${m.bronze}`].filter(Boolean).join(' ') || '–';

  // 커리어 집계
  const im = { gold: 0, silver: 0, bronze: 0 }, tm = { gold: 0, silver: 0, bronze: 0 };
  let fin = 0, dom = 0, intl = 0; const comps = new Set();
  rows.forEach(r => { if (r.medal) im[r.medal]++; if (r.team_medal) tm[r.team_medal]++; if (r.final_score != null) fin++; comps.add(r.competition.name + r.competition.year); if (r.competition.scope === 'international') intl++; else dom++; });
  const gamesInd = rows.filter(r => r.event.team_type === 'individual' && scored(r)).length;

  // 종목별
  const byD = new Map();
  rows.forEach(r => { if (r.event.team_type !== 'individual' || !scored(r)) return; (byD.get(r.event.discipline) || byD.set(r.event.discipline, []).get(r.event.discipline)).push(r); });
  const discRows = [...byD.entries()].map(([k, arr]) => {
    const srt = arr.slice().sort((x, y) => dsort(x).localeCompare(dsort(y)));
  ...
  let h = `<div class="eval-doc"><div class="dos-head">
    <div class="dos-rings">${window.ringsSVG || ''}</div>
    <div class="dos-htxt"><div class="dos-kick">${t('사격 · 베트남 사격연맹')} · ${t('평가 보고서')}</div>
      <h1 class="dos-name">${esc(a.full_name)}${g ? ` <span class="g g-${g}">${GENDER[g]}</span>` : ''}</h1>
      <div class="dos-meta">${by ? `${by}${t('년생')} · ${t('만')} ${NOW_YEAR - by}${t('세')}` : ''}${a.unit ? ` · ${esc(a.unit)}` : ''} · ${a.is_foreign ? esc(a.nationality || '') : t('베트남')}</div></div>
    <div class="dos-asof"><i>${t('기간')}</i><b>${periodLab}</b><s>${t('발급일')} ${dstr}</s></div></div>`;

  if (!rows.length) return h + `<div class="muted" style="padding:20px;text-align:center">${year} ${t('기록이 없습니다.')}</div></div>`;
  ...
  h += `<div class="dos-kpis">
    ...
    ${tile(t('종목'), byD.size, '')}
    ${tile(t('국내') + '/' + t('국제'), dom + '/' + intl, '')}
    ${TR ? tile(t('훈련 세션'), TR.sessions.length, '') : ''}</div>`;
  ...
  if (serByDisc.size) {
    h += `<div class="dos-sec">🔬 ${t('시리즈별 분석')} <span class="dos-help">${t('구간(S1~)별 평균·초반/후반·격차')}</span></div><table class="dos-tab"><thead><tr><th>${t('종목')}</th><th>${t('시리즈 평균')} (S1→)</th><th>${t('초반')}</th><th>${t('후반')}</th><th>${t('격차')}</th><th>${t('경향')}</th></tr></thead><tbody>`;
  ...
  // 월별 추이 (주력 종목: 대회 per10 + 훈련 per10)
  if (mainK) {
    const monthly = new Map();
    rows.forEach(r => { if (r.event.discipline !== mainK || r.event.team_type !== 'individual' || !scored(r)) return; const mo = dsort(r).slice(0, 7); const ns = r.event.n_series || 6; (monthly.get(mo) || monthly.set(mo, { c: [], t: [] }).get(mo)).c.push(r.qual_total / ns); });
    if (TR) TR.sessions.forEach(s => { if ((s.dk || s.disc) !== mainK) return; const mo = s.date.slice(0, 7); if (year !== 'all' && +mo.slice(0, 4) !== year) return; const v = per10t(s); if (v != null) (monthly.get(mo) || monthly.set(mo, { c: [], t: [] }).get(mo)).t.push(v); });
    const mos = [...monthly.keys()].sort();
    if (mos.length >= 2) {
  ...
  // PB 경신 타임라인 (경신 횟수가 가장 많은 종목)
  {
    ...
    if (bestPbs.length >= 2) {
      h += `<div class="dos-sec">🏆 ${t('PB 경신 타임라인')} <span class="dos-help">${t('개인 최고기록 이력')}</span> <span>${esc(DISC[bestK] || bestK)} · ${bestPbs.length}${t('회')}</span></div>
      ...
      h += `<div class="dos-sec">🏆 ${t('개인 최고기록')} <span class="dos-help">${t('개인 최고기록 이력')}</span></div>
  ...
  if (TR && TR.sessions.length) {
    const S = year === 'all' ? TR.sessions : TR.sessions.filter(s => +s.date.slice(0, 4) === year);
    if (S.length) {
      const byDk = new Map(); S.forEach(s => { (byDk.get(s.dk || s.disc) || byDk.set(s.dk || s.disc, []).get(s.dk || s.disc)).push(s); });
      const compP10 = new Map(), cAg = new Map();
      rows.forEach(r => { if (r.event.team_type !== 'individual' || !scored(r)) return; const ns = r.event.n_series || 6; const o = cAg.get(r.event.discipline) || { s: 0, n: 0 }; o.s += r.qual_total / ns; o.n++; cAg.set(r.event.discipline, o); });
      cAg.forEach((o, d) => compP10.set(d, o.s / o.n));
      const mt = S.filter(s => s.is_match).length;
      // 유형별(주력 dk)
      ...
      const CATL = { predict: t('예언사격'), official: t('공식기록'), practice: t('연습사격'), research: t('연구사격'), match: t('실전'), train: t('훈련') };
      h += `<div class="dos-sec">🏋️ ${t('훈련 분석')} <span class="dos-help">${t('훈련 vs 실전 · 유형·요일')}</span> <span>${S.length}${t('세션')} · ${t('훈련')} ${S.length - mt}·${t('실전')} ${mt}</span></div>
        <table class="dos-tab"><thead><tr><th>${t('종목')}</th><th>${t('세션')}</th><th>${t('세션 평균')}</th><th>${t('최고')}</th><th>σ</th><th>${t('대회 평균')}</th><th>${t('대회 대비')}</th><th>${t('추이')}</th></tr></thead><tbody>`;
      [...byDk.entries()].sort((x, y) => y[1].length - x[1].length).forEach(([k, arr]) => {
  ...
  if (year === 'all' && allYears.length > 1) {
    ...
    allYears.slice().reverse().forEach(y => {
      const yr = allRows.filter(r => r.competition.year === y);
      const yi = { gold: 0, silver: 0, bronze: 0 }, yt = { gold: 0, silver: 0, bronze: 0 }; let yf = 0;
      const yv = []; yr.forEach(r => { if (r.medal) yi[r.medal]++; if (r.team_medal) yt[r.team_medal]++; if (r.final_score != null) yf++; if (r.event.team_type === 'individual' && scored(r) && r.event.discipline === mainK) yv.push(r.qual_total); });
      h += `<tr><td class="dos-b">${y}</td><td>${yr.filter(r => r.event.team_type === 'individual' && scored(r)).length}</td><td>${yv.length ? fmt(yv.reduce((s, v) => s + v, 0) / yv.length) : '–'}</td><td>${medStr(yi)}</td><td>${medStr(yt)}</td><td>${yf}${t('회')}</td></tr>`;
    });
    h += `</tbody></table>`;
  }
...
async function openReport(athletes, year) {
  athletes = (athletes || []).filter(Boolean); if (!athletes.length) return;
  let view = document.getElementById('report-view'); if (view) view.remove();
  view = el('div', 'report-view'); view.id = 'report-view';
  view.innerHTML = `<div class="rv-bar no-print">
      <span class="rv-ttl">📊 ${t('평가 보고서')}</span>
      <label class="rv-yr">${t('기간')} <select id="ev-year"></select></label>
      <div class="rv-athsel no-print" id="ev-athsel"></div>
      <button class="rv-print" id="rv-print">🖨️ ${t('인쇄')}</button>
      <button class="rv-close" id="rv-close">${t('닫기')}</button></div>
  ...
  const years = [...yset].sort((x, y) => y - x);
  let cur = (year && year !== 'all' && years.includes(year)) ? year : 'all';
  const ysel = view.querySelector('#ev-year');
  ysel.innerHTML = `<option value="all">${t('전체 기간')}</option>` + years.map(y => `<option value="${y}" ${y === cur ? 'selected' : ''}>${y}</option>`).join('');
  ysel.value = String(cur);
  ysel.onchange = () => { cur = ysel.value === 'all' ? 'all' : +ysel.value; render(); };
  ...
  async function render() {
    const doc = view.querySelector('#rv-doc'); doc.innerHTML = `<div class="muted" style="padding:30px;text-align:center">${t('계산 중…')}</div>`;
    const parts = [];
    for (const a of athletes) { if (!selIds.has(a.id)) continue; parts.push(await buildReportSection(a, careers.get(a.id) || [], cur)); }
    doc.innerHTML = parts.join('') || `<div class="muted" style="padding:30px;text-align:center">${t('선수를 선택하세요.')}</div>`;
  }
  render();
}
...
async function renderAthleteEval(a, rows, year) {
  const scored = r => !r.is_dnf && r.qual_total != null;
  ...
  const now = new Date(); const dstr = `${now.getFullYear()}. ${now.getMonth() + 1}. ${now.getDate()}`;
  const medStr = x => [x.g && `${medalBadge('gold')}${x.g}`, x.s && `${medalBadge('silver')}${x.s}`, x.b && `${medalBadge('bronze')}${x.b}`].filter(Boolean).join(' ') || '–';
  const head = `<div class="rc-head"><div class="rc-rings">${window.ringsSVG || ''}</div>
      <div class="rc-kick">${t('사격 · 베트남 사격연맹')}</div>
      <h1 class="rc-title">${year} ${t('선수 평가 보고서')}</h1>
      <div class="rc-coach">${esc(a.full_name)}${g ? ` <span class="g g-${g}">${GENDER[g]}</span>` : ''} · ${by ? `${t('만')} ${NOW_YEAR - by}${t('세')} · ` : ''}${foreign ? esc(nat || '') : t('베트남')}</div></div>`;
  const yr = rows.filter(r => r.competition.year === year);
  if (!yr.length) return `<div class="eval-doc">${head}<div class="muted" style="padding:24px;text-align:center">${year} ${t('기록이 없습니다.')}</div></div>`;

  const comps = new Set(); const im = { g: 0, s: 0, b: 0 }, tm = { g: 0, s: 0, b: 0 }; let fin = 0;
  yr.forEach(r => { comps.add(r.competition.name); if (r.medal === 'gold') im.g++; else if (r.medal === 'silver') im.s++; else if (r.medal === 'bronze') im.b++; if (r.team_medal === 'gold') tm.g++; else if (r.team_medal === 'silver') tm.s++; else if (r.team_medal === 'bronze') tm.b++; if (r.final_score != null) fin++; });
  const byEv = new Map();
  yr.forEach(r => { if (r.event.team_type !== 'individual' || !scored(r)) return; const k = evKey(r.event); (byEv.get(k) || byEv.set(k, { e: r.event, list: [] }).get(k)).list.push(r); });
  let R = null; try { R = await DB.regionalAnalysis(a.id, year); } catch (e) { }
  const natOf = disc => { if (!R) return null; const dz = R.disciplines.find(d => d.disc === disc); return dz ? `${dz.natRank}/${dz.natN}` : null; };
  const discRows = [...byEv.values()].map(({ e, list }) => { const v = list.map(r => r.qual_total); const avg = v.reduce((s, x) => s + x, 0) / v.length; return { e, n: list.length, avg, best: Math.max(...v), worst: Math.min(...v), nat: natOf(e.discipline) }; }).sort((x, y) => x.e.discipline.localeCompare(y.e.discipline));
  let stages = []; try { stages = await DB.stageAnalysis(a.id, year); } catch (e) { }
  const stageTxt = stages.map(dz => { if (dz.stages.length < 2) return null; const w = dz.stages.reduce((m, s) => s.avg < m.avg ? s : m); return `${esc(DISC[dz.disc] || dz.disc)} · <b>${t(w.key)}</b> ${w.avg.toFixed(1)}`; }).filter(Boolean);
  const byComp = new Map(); yr.forEach(r => { (byComp.get(r.competition.name) || byComp.set(r.competition.name, { c: r.competition, items: [] }).get(r.competition.name)).items.push(r); });
  const compList = [...byComp.values()].sort((x, y) => (y.c.date_start || '').localeCompare(x.c.date_start || ''));
  const main = discRows.slice().sort((x, y) => (x.nat ? +x.nat.split('/')[0] : 999) - (y.nat ? +y.nat.split('/')[0] : 999))[0];
  let comment = `${year}${t('년')} ${comps.size}${t('개 대회')} ${yr.filter(scored).length} ${t('경기')} ${t('출전')}. `;
  if (main) comment += `${t('주력')} ${esc(DISC[main.e.discipline] || main.e.discipline)}${main.nat ? ` ${t('전국')} ${main.nat.split('/')[0]}${t('위')}` : ''} (${t('평균')} ${main.avg.toFixed(1)}). `;
  if (im.g + im.s + im.b) comment += `${t('개인 메달')} ${im.g + im.s + im.b}${t('개')}. `;
  if (stageTxt.length) comment += `${t('보강')}: ${stageTxt[0].replace(/<\/?b>/g, '')}.`;

  return `<div class="eval-doc">${head}
    <div class="rc-sum">
      <div><i>${t('참가 대회')}</i><b>${comps.size}</b></div>
      <div><i>${t('경기')}</i><b>${yr.filter(scored).length}</b></div>
      <div class="hl"><i>${t('개인 메달')}</i><b>${im.g + im.s + im.b}</b><span>${medStr(im)}</span></div>
      <div><i>${t('단체 메달')}</i><b>${tm.g + tm.s + tm.b}</b><span>${medStr(tm)}</span></div>
      <div><i>${t('결선 진출')}</i><b>${fin}${t('회')}</b></div>
    </div>
    ${discRows.length ? `<div class="rc-sech">🎯 ${t('종목별 성적')}</div>
      <table class="rc-tab"><thead><tr><th>${t('종목')}</th><th>${t('경기')}</th><th>${t('평균')}</th><th>${t('최고')}</th><th>${t('최저')}</th><th>${t('전국')}</th></tr></thead><tbody>` +
      discRows.map(d => `<tr><td class="rc-nm">${esc(eventLabel(d.e))} ${ageChip(d.e)}</td><td>${d.n}</td><td><b>${d.avg.toFixed(1)}</b></td><td>${d.best}</td><td>${d.worst}</td><td>${d.nat || '–'}</td></tr>`).join('') + `</tbody></table>` : ''}
    ${stageTxt.length ? `<div class="rc-sech">📊 ${t('단계 약점')}</div><ul class="eval-stage">` + stageTxt.map(x => `<li>${x}</li>`).join('') + `</ul>` : ''}
    <div class="rc-sech">🏅 ${t('대회별 성적')}</div>` +
    compList.map(({ c, items }) => `<div class="rc-disc"><div class="rc-disc-h">${(c.date_start || '').replace(/-/g, '.')} ${esc(c.name)} <span class="scope ${c.scope}">${SCOPE[c.scope] || ''}</span></div>
      <table class="rc-tab"><tbody>` + items.map(r => `<tr><td class="rc-nm">${esc(eventLabel(r.event))}</td><td>${r.qual_total != null ? t('본선') + ' ' + num(r.qual_total) : ''}${r.final_score != null ? ' · ' + t('결선') + ' ' + r.final_score : ''}</td><td class="rc-md">${r.medal ? medalBadge(r.medal) : ''}${r.team_medal ? medalBadge(r.team_medal) + `<span class="tmlbl">${t('단체')}</span>` : ''}${r.placement ? ` ${r.placement}${t('위')}` : ''}</td></tr>`).join('') + `</tbody></table></div>`).join('') +
    `<div class="rc-sech">📝 ${t('총평')}</div><div class="eval-comment">${comment}</div>
    <div class="rc-foot"><div class="rc-note">${t('본 보고서는 베트남 사격연맹 공개 기록을 기준으로 자동 작성되었습니다.')} · ${t('발급일')} ${dstr}</div>
      <div class="rc-sign"><span class="rc-signname">RYONG</span><span class="rc-signlab">${t('작성')}</span></div></div>
  </div>`;
}
...
async function buildCareer(a, rows, detail) {
  ...
  const repBtn = el('button', 'eval-btn dos-btn', `📊 ${t('평가 보고서')}`);
  repBtn.onclick = () => openReport([{ id: a.id, full_name: a.full_name, gender: g, birth_year: by, birth_date: a.birth_date || a0.birth_date, is_foreign: foreign, nationality: nat, identity_key: a.identity_key || a0.identity_key, unit: a.units || a.unit }], 'all');
